...
def lower(code: list[int], vm_id: int = 0) -> Program:
    ...
    for kind in sorted({_recipe(op) for _, op in pending}):
        ...
        if comparison:
            ...
            compare_strings(out)
            out[string_branch] = I(Op.SELECT, 320, string_branch + 2, len(out) + 1)
            ...
            compare_floats(out)
            out[branch] = I(Op.SELECT, 176, branch + 2, len(out) + 1)
        for digit in range(16):
    ...
    return Program(entries, out, len(pending) + len(jumps) + boolean_sites, vm_id, recipes)
